...
impl UdpEndpoint {
    pub fn new(config: UdpEndpointConfig) -> Self {
        ...
        UdpEndpoint {
            ...
            subscribers: HashMap::<u32, Vec<Recipient<Arc<Envelope>>>>::new(),
            // broker_addr: None,
            // pinger,
            // send_socket: Arc::new(Mutex::new(None)),
        }
...
impl Message<Subscribe> for UdpEndpoint {
    ...
    async fn handle(&mut self, msg: Subscribe, _ctx: &mut Context<Self, Self::Reply>) {
        let recipients = self.subscribers.entry(msg.msg_type).or_insert_with(Vec::new);
        recipients.push(msg.recipient);
        info!(
            "UdpEndpoint '{}' subscribed recipient for msg_type {}",
            self.config.name, msg.msg_type
        );
    }
}