//! broker_actix — Limeros message broker built on actix actors.
//!
//! Architecture:
...
#[derive(Parser, Debug)]
#[command(about = "Limeros broker — actix-based message routing")]
struct Args {
    /// HCL config path.
...
fn parse_subscriptions(cfg: &RobotConfig) -> Vec<Subscription> {
    ...
    for ep in cfg.endpoints.values() {
        for sub in &ep.subscribes {
            let src = sub.src.as_deref().and_then(|v| {
                // Try parsing as FNV u32 first, then as name
                v.parse::<u32>().ok().or_else(|| Some(common::fnv1a_32(v)))
            });
            let msg_type = sub
                .msg_type
                .as_deref()
...
async fn main() -> anyhow::Result<()> {
    ...
    // Start Router actor
    let router_actor = Router::new(broker_id, subscriptions).start();
    ...
    let udp_actor = UdpActor::new(router_actor.clone()).start();
    udp_actor.do_send(StartUnicast {
        port: broker_port,
        bind_addr,
    });

    let multicast_actor = MulticastActor::new(router_actor.clone(), udp_actor.clone()).start();
    multicast_actor.do_send(StartMulticast {
        group: mc_group,
        port: mc_port,
        bind_addr,
    });
    ...
    for (name, ep_cfg) in &cfg.endpoints {
        if let Some(ref transport) = ep_cfg.transport {
            ...
            let _serial = SerialActor::new(transport.clone(), router_actor.clone()).start();
            // Serial actor registers itself with the router on start
        }
    }
