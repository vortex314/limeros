...
Result<Void> FrameEncoder::encode_str(const char* str) {
    ...
    else if (len > 23 && len < 256) {
        RET_ERR(write_byte(0x78));  // Major type 3 (text string)
        RET_ERR(write_byte(len));
    }
    else if (len > 255 && len < 65536) {
        RET_ERR(write_byte(0x79));  // Major type 3 (text string)
        RET_ERR(write_byte((len >> 8) & 0xFF));
        RET_ERR(write_byte(len & 0xFF));
    }
    else
        return Result<Void>::Err(ENOSPC);
...
Result<int8_t> FrameDecoder::decode_int8() {
    auto r = decode_int32();
    if (r.is_err()) {
        return Result<int8_t>::Err(EINVAL, "Buffer decode_int8 fails");
    }
    return Result<int8_t>::Ok(r.unwrap());
}

Result<uint8_t> FrameDecoder::decode_uint8() {
    auto r = decode_uint32();
    if (r.is_err()) {
        return Result<uint8_t>::Err(EINVAL, "Buffer decode_int8 fails");
    }
    return Result<uint8_t>::Ok(r.unwrap());
}

Result<bool> FrameDecoder::decode_bool() {
    auto r = peek_type();
    if (r.is_err()) {
        return Result<bool>::Err(EINVAL, "Buffer decode_bool peek_next fails");
    }
    uint8_t header = r.unwrap();
    if (header == 0xF5) {
        read_next();
    ...
    return Result<bool>::Err(EINVAL, "Not a CBOR bool");
}
